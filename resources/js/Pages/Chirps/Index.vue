<template>
    ...
    <AuthenticatedLayout>
        <div class="max-w-3xl mx-auto p-4 sm:p-6 lg:p-8">
            ...
            <div
                class="w-full p-4 mb-10 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-10 dark:bg-white dark:border-gray-300">
                <form class="space-y-3" action="#">
                    ...
                    <div class="flex" :class="$page.props.auth.user.id === chirp.user.id ? 'justify-end' : 'justify-start'"
                        v-for="chirp in chirps" :key="chirp.id">
                        <div class="flex flex-col">
                            <div class="flex"
                                :class="$page.props.auth.user.id === chirp.user.id ? 'justify-end' : 'justify-start'">
                            ...
                            <div class="flex items-center">
                                ...
                                <div class="flex place-items-center"
                                    :class="$page.props.auth.user.id !== chirp.user.id ? 'flex-row-reverse' : ''">
                                    <small class="text-xs text-gray-500 px-3">{{ timeMessage(chirp.created_at) }}</small>
                                    <span class="p-2 py-1.5 px-4 rounded-full text-sm font-thin"
                                        :class="$page.props.auth.user.id === chirp.user.id ? 'dark:text-white bg-blue-600' : ' text-gray-900 border-2 border-gray-400'">{{ chirp.message }}</span>
                                </div>
                                <!-- </div> -->
                                    ...
                                    <template #trigger>
                                        <button>
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                fill="currentColor" class="bi bi-three-dots-vertical text-gray-400"
                                                viewBox="0 0 16 16">
                                                <path
                                ...
                                <form v-if="editing"
                                    ...
                                    <input v-model="chirp.message" class="p-2 px-4 rounded-full text-sm font-thin"
                                        :class="$page.props.auth.user.id === chirp.user.id ? 'dark:text-white bg-blue-600' : ' text-gray-900 border-2 border-gray-400'">
                    ...
                    <div>
                        <span v-show="typing" class="py-1.5 px-4 rounded-full text-sm font-thin">{{ user }} is typing
                            ...</span>
                    </div>
            ...
            <form @submit.prevent="submmitForm">
                <textarea v-model="form.message" @input="typeMessage" placeholder="What's on your mind?"
